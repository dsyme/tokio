...
async fn timeout_repeating_immediate_items() {
    let stream = stream::iter(vec![1, 2, 3, 4, 5]).timeout_repeating(interval(ms(100)));
...
async fn timeout_repeating_all_timeout() {
    ...
    let stream = futures_stream::pending::<i32>().timeout_repeating(interval(ms(50)));
...
async fn timeout_repeating_fast_interval() {
    ...
    let stream = futures_stream::pending::<i32>().timeout_repeating(interval(ms(10)));
...
async fn try_next_with_parsing() {
    ...
    let mut stream =
        stream::iter(vec!["1", "not_a_number", "3", "also_invalid", "5"]).map(|s| s.parse::<i32>());
